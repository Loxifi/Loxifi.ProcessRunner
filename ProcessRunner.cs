...
namespace Loxifi
{
    ...
    public static class ProcessRunner
    {
        ...
        {
            string commandLine = settings != null ? ArgumentService.Parse(settings.Arguments) : throw new ArgumentNullException(nameof(settings));
            ...
            try
            {
                ...
                else if (!CreateProcessW($"{fileName}", $"\"{fileName}\" {commandLine}", new SECURITY_ATTRIBUTES(), new SECURITY_ATTRIBUTES(), true, creationFlags, lpEnvironment, settings.WorkingDirectory, ref startupInfo, out p))
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
                ...
                RunningProcess runningProcess = new(settings, commandLine);
                ...
                async Task CleanUp()
                {
                    DontCare(cancellationToken.Cancel);
                    await DontCare(readOutput);
                    await DontCare(readError);
                    DontCare(() => CloseHandle(p.hProcess));
                    DontCare(() => CloseHandle(p.hThread));
                }
        ...
        private static extern bool CreateProcessW(string applicationName, string commandLine, SECURITY_ATTRIBUTES lpProcessAttributes, SECURITY_ATTRIBUTES lpThreadAttributes, bool bInheritHandles, PROCESS_CREATION_FLAGS creationFlags, IntPtr environment, string currentDirectory, ref STARTUPINFO lpStartupInfo, out PROCESS_INFORMATION lpProcessInformation);
        ...
        private static void DontCare(Action toInvoke)
        {
            try
            {
                toInvoke.Invoke();
        ...
        private static async void DontCare(Func<Task> toInvoke)
        {
            try
            {
        ...
        private static async Task DontCare(Task toInvoke)
        {
            try
            {
                await toInvoke;
